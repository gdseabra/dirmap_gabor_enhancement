...
class DirmapLitModule(LightningModule):
    def __init__(
        self,
        net: torch.nn.Module,
        optimizer: Callable,
    ...
    def model_step(self, batch: Tuple[torch.Tensor, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        pred_dirmap, gabor = self.forward(x)
    ...
    def training_step(
    ...
    ) -> torch.Tensor: # <- Note que agora ele retorna um Tensor
        ...
        # --- MUDANÇA PRINCIPAL ---
        # Retorne a loss para o Lightning fazer a otimização
        return loss
    ...
    def predict_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        if not self.use_patches:
            dirmap_pred, dirmap_gabor = self.forward(data)
